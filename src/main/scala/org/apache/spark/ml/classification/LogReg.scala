 ...
 */
private[ml] trait LogRegParams extends ProbabilisticClassifierParams
 ...
 */
@Since("1.2.0")
class LogReg @Since("1.2.0") (
                               @Since("1.4.0") override val uid: String)
  extends ProbabilisticClassifier[Vector, LogReg, LogRegModel]
    with LogRegParams with DefaultParamsWritable with Logging {
  ...
  private var optInitialModel: Option[LogRegModel] = None

  private[spark] def setInitialModel(model: LogRegModel): this.type = {
    this.optInitialModel = Some(model)
    this
  }

  override protected[spark] def train(dataset: Dataset[_]): LogRegModel = {
  ...
  protected[spark] def train(
                              dataset: Dataset[_],
                              handlePersistence: Boolean): LogRegModel = {
...
//    instances.unpersist()
    ...
    val data = instances.mapPartitions { it =>
      it.grouped(blockSize).map { block =>
        ...
        val examples = new Array[Double](block.length * numFeatures)
        val labels = new Array[Int](block.length)
        val weights = Nd4j.ones(block.length)
        block.zipWithIndex.foreach { case (instance, j) =>
          labels(j) = instance.label.toInt
          System.arraycopy(instance.features.toArray, 0, examples, j * numFeatures, numFeatures)
        }
...
//    data.persist(StorageLevel.MEMORY_AND_DISK_SER)
    ...
    val (coefficientMatrix, interceptVector, objectiveHistory) = {
      ...
      if ($(fitIntercept) && isConstantLabel) {
        ...
        val bcFeaturesStd = instances.context
          .broadcast(Nd4j.create(featuresStd).reshape(numFeatures, 1))
        val costFun = new LogCostFun(data, numClasses, $(fitIntercept),
    ...
    val model = copyValues(new LogRegModel(uid, coefficientMatrix, interceptVector,
  ...
  @Since("1.4.0")
  override def copy(extra: ParamMap): LogReg = defaultCopy(extra)
}

@Since("1.6.0")
object LogReg extends DefaultParamsReadable[LogReg] {

  @Since("1.6.0")
  override def load(path: String): LogReg = super.load(path)
 ...
 */
@Since("1.4.0")
class LogRegModel private[spark] (
                                   @Since("1.4.0") override val uid: String,
  ...
  extends ProbabilisticClassificationModel[Vector, LogRegModel]
    with LogRegParams with MLWritable {
  ...
  @Since("1.4.0")
  override def copy(extra: ParamMap): LogRegModel = {
    val newModel = copyValues(new LogRegModel(uid, coefficientMatrix, interceptVector,
   ...
   */
  @Since("1.6.0")
  override def write: MLWriter = new LogRegModel.LogRegModelWriter(this)
}


@Since("1.6.0")
object LogRegModel extends MLReadable[LogRegModel] {

  @Since("1.6.0")
  override def read: MLReader[LogRegModel] = new LogRegModelReader

  @Since("1.6.0")
  override def load(path: String): LogRegModel = super.load(path)

  /** [[MLWriter]] instance for [[LogisticRegressionModel]] */
  private[LogRegModel]
  class LogRegModelWriter(instance: LogRegModel)
  ...
  private class LogRegModelReader extends MLReader[LogRegModel] {

    /** Checked against metadata when loading model */
    private val className = classOf[LogRegModel].getName

    override def load(path: String): LogRegModel = {
      ...
      val model = if (major.toInt < 2 || (major.toInt == 2 && minor.toInt == 0)) {
        ...
        new LogRegModel(metadata.uid, coefficientMatrix,
        ...
        coefficientMatrix: Matrix, isMultinomial: Boolean) = data
          .select("numClasses", "numFeatures", "interceptVector", "coefficientMatrix",
            "isMultinomial").head()
        new LogRegModel(metadata.uid, coefficientMatrix, interceptVector,
          numClasses, isMultinomial)
      }
...
private class LogAggregator(
                             bcCoefficients: Broadcast[INDArray],
                             bcIntercepts: Broadcast[INDArray],
                             bcFeaturesStd: Broadcast[INDArray],
                             numClasses: Int,
  ...
  private val coefficientSize = bcCoefficients.value.length()
  private val numCoefficientSets = if (multinomial) numClasses else 1
  if (multinomial) {
    require(numClasses ==  coefficientSize / numFeatures, s"The number of " +
      s"coefficients should be ${numClasses * numFeatures} but was $coefficientSize")
  } else {
  ...
  private val cumGradient = Nd4j.zeros(numClasses, numFeatures)
  private val cumInterceptGradient = Nd4j.zeros(numClasses, 1)

  if (multinomial && numClasses <= 2) {
    println(s"Multinomial logistic regression for binary classification yields separate " +
  ...
  def multinomialUpdateInPlace(
                                features: INDArray,
                                weight: INDArray,
                                labels: Array[Int]): Unit = {
    require(features.shape()(0) == numFeatures)
    val localFeaturesStd = bcFeaturesStd.value
    val localCoefficients = bcCoefficients.value
    val localIntercepts = bcIntercepts.value
    val outMat = FeatureUtil.toOutcomeMatrix(labels, numClasses).transpose


    val normalized = features.divColumnVector(localFeaturesStd)
    // margins is (numClasses x numExamples)
    val margins = localCoefficients.mmul(normalized)
    margins.addiColumnVector(localIntercepts)
    val labelMargins = margins.mul(outMat).sum(0)
    // TODO: overflow correction
    val maxMargins = margins.max(0)
    margins.subiRowVector(maxMargins)
    labelMargins.subi(maxMargins)
    Nd4j.getExecutioner.execAndReturn(new Exp(margins))
    val sums = margins.sum(0)

    margins.diviRowVector(sums)
    margins.subi(outMat)
    val thisInterceptGradient = margins.sum(1)
    val thisGradient = margins.mmul(normalized.transpose)
    Nd4j.getExecutioner.execAndReturn(new Log(sums))
    sums.subi(labelMargins)
    val loss = sums.sumNumber().doubleValue()

    this.cumGradient.addi(thisGradient)
    this.cumInterceptGradient.addi(thisInterceptGradient)
    this.lossSum += loss
  }
   ...
   */
  def add(instance: (Array[Int], INDArray, INDArray)): this.type = {
    instance match { case (labels: Array[Int], weights: INDArray, features: INDArray) =>
      require(features.shape()(0) == numFeatures)
      require(features.shape()(1) == labels.length)
      require(labels.length == weights.length)

      if (multinomial) {
        multinomialUpdateInPlace(features, weights, labels)
      } else {
        throw new IllegalArgumentException("no binary supported")
      }
      weightSum += weights.sumNumber().doubleValue()
      this
    }
  }
   ...
   * @return This LogisticAggregator object.
   */
  def merge(other: LogAggregator): this.type = {
    ...
    if (other.weightSum != 0.0) {
      ...
      val localThisGradientSumArray = this.cumGradient
      val localOtherGradientSumArray = other.cumGradient
      localThisGradientSumArray.addi(localOtherGradientSumArray)
      this.cumInterceptGradient.addi(other.cumInterceptGradient)
    }
    this
  }
  ...
  def gradient: INDArray = {
    require(weightSum > 0.0, s"The effective number of instances should be " +
      s"greater than 0.0, but $weightSum.")
    this.cumGradient.div(weightSum)
  }

  def interceptGradient: INDArray = {
    require(weightSum > 0.0, s"The effective number of instances should be " +
      s"greater than 0.0, but $weightSum.")
    this.cumInterceptGradient.div(weightSum)
  }
}
 ...
 */
private class LogCostFun(
                          instances: RDD[(Array[Int], INDArray, INDArray)],
                          numClasses: Int,
                          fitIntercept: Boolean,
  ...
  def arrayToND4j(arr: Array[Double], numFeatures: Int): (INDArray, INDArray) = {
    val coefArray = new Array[Double](numClasses * numFeatures)
    val interceptArray = new Array[Double](numClasses)
    System.arraycopy(arr, 0, coefArray, 0, numClasses * numFeatures)
    System.arraycopy(arr, numClasses * numFeatures, interceptArray, 0, numClasses)
    val coef = Nd4j.create(coefArray).reshape(numFeatures, numClasses).transpose
    val intercept = Nd4j.create(interceptArray).reshape(numClasses, 1)
    (coef, intercept)
  }
  ...
  override def calculate(coefficients: BDV[Double]): (Double, BDV[Double]) = {
    ...
    val (coefs, intercepts) = arrayToND4j(coefficients.data, numFeatures)
    val bcCoeffs = instances.context.broadcast(coefs)
    val bcIntercepts = instances.context.broadcast(intercepts)
    val numCoefficientSets = if (multinomial) numClasses else 1
    val numFeaturesPlusIntercept = if (fitIntercept) numFeatures + 1 else numFeatures

    val logisticAggregator = {
      val seqOp = (c: LogAggregator, instance: (Array[Int], INDArray, INDArray)) => c.add(instance)
      val combOp = (c1: LogAggregator, c2: LogAggregator) => c1.merge(c2)

      instances.treeAggregate(
        new LogAggregator(bcCoeffs, bcIntercepts, bcFeaturesStd, numClasses, fitIntercept,
          multinomial)
      )(seqOp, combOp, aggregationDepth)
    }

    val coefGrad = logisticAggregator.gradient
    val intGrad = logisticAggregator.interceptGradient
    ...
    if (standardization) {
      ...
      coefGrad.addi(coefs.div(fstdSquared).muli(regParamL2))
      val coefSq = coefs.add(coefs)
      regVal = coefSq.divi(fstdSquared).sumNumber().doubleValue()
    }
    regVal *= 0.5 * regParamL2
    ...
    System.arraycopy(interceptArr, 0, totalCoefArray, coefArr.length, interceptArr.length)
